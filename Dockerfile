...
COPY ./store/main.sql ./store/main.sql

# Собранный ранее сервер
COPY --from=build /dir/db-forum/apiserver .

#
# Запускаем PostgreSQL и сервер
#
CMD service postgresql start && ./apiserver --scheme=http --port=5000 --host=0.0.0.0 --database=postgres://docker:docker@localhost/docker
